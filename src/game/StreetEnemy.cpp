...
void StreetEnemy::TakeDamage(int damage,Vector3D knockback){if(!active||state==StreetEnemyState::Defeat)return;hp=std::max(0,hp-damage);velocity=knockback;if(hp==0){state=StreetEnemyState::Defeat;stateTimer=.85f;if(animator.texture.id!=0)animator.Play({10,11,.14f,false});else{animator.isFinished=true;animator.isPlaying=false;}}else{state=StreetEnemyState::Hit;stateTimer=.30f;if(animator.texture.id!=0)animator.Play({0,0,.09f,false});}}
