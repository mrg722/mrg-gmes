...
private:
    ...
    struct TitanX {
        Vector3D pos{5350, 570, 0};
        int hp{1500};
        int maxHp{1500};
        int phase{1};
        float attackTimer{1.2f};
        float powerTimer{2.2f};
        float shieldTimer{0.8f};
        float invuln{0};
        float dashTimer{0};
        float elapsed{0};
        bool blocking{false};
    ...
    float cameraX{640};
    bool scenarioGatekeeperSpawned{false};
    ...
    void SpawnBossProjectile(int damage, float speed);
