...
void Player::Update(float dt) {
	...
	if (state == PlayerState::Attack) { attackElapsed += dt; stateTimer -= dt; if (attackType == AttackType::Energy && !energyReleased && attackElapsed >= .22f) energyReleased = true; if (stateTimer <= 0 || animator.isFinished) { attackType = AttackType::None; SetState(PlayerState::Idle); } return; }
	...
	auto beginAttack = [&](AttackType type, float duration, int cleanStart, int cleanEnd, int legacyStart, int legacyEnd, float frameDuration, std::vector<int> sequence = {}) { state = PlayerState::Attack; attackType = type; attackElapsed = 0; attackDuration = duration; stateTimer = duration; hasHit = false; energyReleased = false; int start = animator.normalizedAtlas ? cleanStart : legacyStart; int end = animator.normalizedAtlas ? cleanEnd : legacyEnd; if (!sequence.empty() && animator.normalizedAtlas) animator.Play({start, end, frameDuration, false, sequence}); else animator.Play({start, end, frameDuration, false}); };
	if (IsKeyPressed(KEY_J)) { comboStep = comboWindow > 0 ? (comboStep + 1) % 3 : 0; beginAttack(AttackType::Punch, .30f, 8, 9, 5, 9, .15f, {8,9}); AudioSystem::Get().Play(Sfx::Punch); return; }
	if (IsKeyPressed(KEY_K)) { comboStep = 3; beginAttack(AttackType::Kick, .36f, 10, 11, 10, 14, .18f, {10,11}); AudioSystem::Get().Play(Sfx::Kick); return; }
	if (IsKeyPressed(KEY_L) && sp >= 20) { sp -= 20; beginAttack(AttackType::Energy, .54f, 12, 13, 5, 9, .135f, {12,13,13,12}); AudioSystem::Get().Play(Sfx::EnergyCharge); return; }
...
bool Player::AttackIsActive() const { if (state != PlayerState::Attack) return false; if (attackType == AttackType::Punch) return attackElapsed >= .08f && attackElapsed <= .23f; if (attackType == AttackType::Kick) return attackElapsed >= .10f && attackElapsed <= .29f; return attackType == AttackType::Energy && attackElapsed >= .22f && attackElapsed <= .46f; }
...
void Player::Draw() const {
	Vector2 p = position.ToScreen(); float scale = DepthScale(position.y); float spriteScale = animator.normalizedAtlas ? 1.20f * scale : .76f * scale;
	DrawEllipse((int)p.x, (int)p.y, 30 * scale, 8 * scale, {0,0,0,145});
	if (isRageMode) {
		const float t = (float)GetTime(); const float pulse = 1.0f + 0.08f * std::sin(t * 12.0f); const float cx = p.x; const float cy = p.y - 66.0f * scale;
		DrawEllipse((int)cx, (int)cy, 48.0f * scale * pulse, 88.0f * scale * pulse, {35,105,255,42});
		DrawEllipseLines((int)cx, (int)cy, 42.0f * scale * pulse, 82.0f * scale * pulse, {65,205,255,170});
		DrawEllipseLines((int)cx, (int)cy, 50.0f * scale * pulse, 94.0f * scale * pulse, {30,95,255,105});
		for (int i = 0; i < 10; ++i) { const float a = t * (1.8f + i * 0.08f) + i * 0.6283185f; const float innerX = cx + std::cos(a) * 28.0f * scale; const float innerY = cy + std::sin(a) * 44.0f * scale; const float outerX = cx + std::cos(a + 0.18f) * (39.0f + 6.0f * std::sin(t * 5.0f + i)) * scale; const float outerY = cy + std::sin(a + 0.18f) * (70.0f + 8.0f * std::sin(t * 4.0f + i)) * scale; DrawLine((int)innerX, (int)innerY, (int)outerX, (int)outerY, {70,205,255,145}); }
	}
	if (IsBlocking() || shield < maxShield) { float ratio = std::clamp((float)shield / maxShield, 0.f, 1.f); float pulse = (1.f - ratio) * 3.f + std::sin((float)GetTime() * 10.f) * (ratio < .3f ? 2.f : .5f); Color shieldColor = ratio < .3f ? Color{255,70,70,34} : Color{45,180,255,28}; DrawEllipse((int)p.x, (int)(p.y - 63 * scale), (38 + pulse) * scale, (76 + pulse) * scale, shieldColor); DrawEllipseLines((int)p.x, (int)(p.y - 63 * scale), (38 + pulse) * scale, (76 + pulse) * scale, ratio < .3f ? Color{255,90,80,230} : Color{80,215,255,210}); DrawEllipseLines((int)p.x, (int)(p.y - 63 * scale), (33 + pulse) * scale, (70 + pulse) * scale, {180,245,255,125}); }
	if (animator.texture.id != 0) { Color tint = state == PlayerState::Hit ? Color{255,190,190,255} : state == PlayerState::Block ? Color{175,220,255,255} : isRageMode ? Color{185,225,255,255} : WHITE; animator.Draw(p, spriteScale, facing == Facing::Left, tint); } else { Color tint = state == PlayerState::Hit ? RED : state == PlayerState::Attack ? YELLOW : state == PlayerState::Block ? Color{80,190,255,255} : BLUE; DrawRectangle((int)(p.x - 18 * scale), (int)(p.y - 68 * scale), (int)(36 * scale), (int)(68 * scale), tint); }
	if (IsBlocking()) { const char* label = shield == 0 ? "ESCUDO ROTO" : "BLOQUEO"; DrawText(label, (int)p.x - MeasureText(label, 12) / 2, (int)p.y - 154, 12, {120,215,255,230}); }
	if (state == PlayerState::Attack && attackType == AttackType::Energy) { const float charge = std::clamp(attackElapsed / .22f, 0.f, 1.f); const float dir = facing == Facing::Right ? 1.0f : -1.0f; DrawCircleLines((int)(p.x + dir * 30), (int)(p.y - 74), 11 + 15 * charge, {50,210,255,135}); DrawCircleLines((int)(p.x + dir * 39), (int)(p.y - 74), 7 + 10 * charge, {150,245,255,105}); }
}
}