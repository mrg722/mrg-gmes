...
void Player::Draw() const {
    ...
    if (animator.texture.id != 0) {
        Color tint = WHITE;
        if (state == PlayerState::Hit || state == PlayerState::Knockdown) tint = {255, 190, 190, 255};
        else if (state == PlayerState::Block) tint = {175, 220, 255, 255};
        else if (isRageMode) tint = {255, 214, 188, 255};
        else if (damageBuffTimer > 0.0f) tint = {255, 226, 190, 255};
        animator.Draw(p, spriteScale, facing == Facing::Left, tint);
    } else {
