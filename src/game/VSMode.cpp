...
void VSMode::Init(){flow=VSFlow::Select;stage=0;scenario=0;enemyCount=1;cursor=0;exitRequested=false;playerDefeated=false;enemyTypes={StreetEnemyType::Punk,StreetEnemyType::Brute,StreetEnemyType::Charger,StreetEnemyType::Enforcer};enemies.clear();player.Reset();player.position={360,585,0};shake=0;}
...
const char* VSMode::ScenarioText()const{
 ...
 return kStage3Scenarios[std::clamp(scenario,0,2)];
}
...
void VSMode::ResetFight(){
 player.Reset();player.position={360,585,0};playerDefeated=false;shake=0;enemies.clear();
...
void VSMode::Update(float dt){
 ...
 if(flow==VSFlow::Select){
  ...
  if(IsKeyPressed(KEY_LEFT)||IsKeyPressed(KEY_RIGHT)){
   ...
   if(cursor==0){stage=(stage+dir+3)%3;scenario=std::clamp(scenario,0,ScenarioCount()-1);}
   ...
   else{int slot=cursor-3;enemyTypes[(size_t)slot]=NextEnemyType(enemyTypes[(size_t)slot],dir);}
  }
...
void VSMode::DrawHud()const{
 DrawRectangle(16,16,530,122,{3,7,11,232});
 const Texture2D portrait=AssetManager::Get().GetTexture("rayden_clean");
 if(portrait.id)DrawTexturePro(portrait,{0,0,96,96},{28,28,70,70},{0,0},0,WHITE);
 DrawRectangleLines(25,25,76,76,{70,200,235,160});
 DrawText("RAYDEN CRUZ",112,25,20,{220,235,240,255});
 DrawText("VIDA",112,53,12,WHITE);DrawRectangle(162,53,250,12,{24,27,31,240});DrawRectangle(162,53,(int)(250.f*player.hp/std::max(1,player.maxHp)),12,{225,65,65,255});
 DrawText("SP",112,76,12,{120,215,255,255});DrawRectangle(162,76,116,8,{22,26,31,240});DrawRectangle(162,76,(int)(116.f*player.sp/std::max(1,player.maxSp)),8,{55,190,245,255});
 DrawText("FURIA",294,76,12,{255,205,75,255});DrawRectangle(340,76,116,8,{28,25,21,240});DrawRectangle(340,76,(int)(116.f*player.rage/std::max(1,player.maxRage)),8,player.isRageMode?Color{255,75,60,255}:Color{255,190,60,255});
 DrawText(TextFormat("ESCUDO %d%%",(int)(100.f*player.shield/std::max(1,player.maxShield))),112,100,12,{105,215,255,235});
 DrawText(TextFormat("STAGE %d  //  %s",stage+1,ScenarioText()),294,100,11,{175,200,210,230});
 DrawRectangle(818,16,446,122,{3,7,11,220});DrawText(StageName(),840,25,18,{255,205,75,255});DrawText(TextFormat("ENEMIGOS %d/4",enemyCount),840,52,13,WHITE);
...
void VSMode::DrawSelection()const{
 ...
 const int y[]={165,214,263,312,361,410,459};const Color active={255,215,80,255};
 const char* labels[]={"STAGE","ESCENARIO","CANTIDAD","ENEMIGO 1","ENEMIGO 2","ENEMIGO 3","ENEMIGO 4"};
 for(int i=0;i<kFieldCount;++i){bool selected=cursor==i;DrawRectangle(335,y[i]-8,610,38,selected?Color{20,28,34,230}:Color{8,15,21,190});DrawRectangleLines(335,y[i]-8,610,38,selected?Color{255,205,75,210}:Color{70,95,105,90});DrawText(labels[i],360,y[i],16,selected?active:WHITE);}
 DrawText(StageName(),600,y[0],16,{190,220,230,255});DrawText(ScenarioText(),600,y[1],16,{190,220,230,255});DrawText(TextFormat("%d ENEMIGO%s",enemyCount,enemyCount==1?"":"S"),600,y[2],16,{190,220,230,255});
 for(int i=0;i<4;++i){bool enabled=i<enemyCount;DrawText(EnemyTypeName(enemyTypes[(size_t)i]),600,y[3+i],16,enabled?Color{190,220,230,255}:Color{85,95,100,130});}
 DrawText("↑/↓ CAMPO    ←/→ CAMBIAR    ENTER/J INICIAR",391,530,14,{170,195,205,245});DrawText("ESC VOLVER AL MENU",485,559,13,{130,155,165,220});
}
void VSMode::DrawFight()const{
 ...
 DrawHud();DrawRectangle(260,657,760,45,{3,7,11,225});DrawText("J GOLPE   K PATADA   L ENERGIA   B BLOQUEO   SHIFT DASH   SPACE FURIA",296,669,12,{170,200,210,240});DrawText("R REINICIAR   ESC CONFIGURACION",489,687,11,{135,155,165,220});
