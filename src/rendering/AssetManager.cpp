...
bool NearWhite(const Color& c){
    const int mx=std::max({(int)c.r,(int)c.g,(int)c.b});
    const int mn=std::min({(int)c.r,(int)c.g,(int)c.b});
    return c.a>0&&mn>=235&&(mx-mn)<=18;
}
...
Texture2D LoadEnemyTexture(const char* key,const std::vector<std::string>& candidates){
    ...
    const auto enqueue=[&](int x,int y){
        const int idx=y*w+x;
        if(!visited[(size_t)idx]&&NearWhite(GetImageColor(image,x,y))){visited[(size_t)idx]=1;queue.push(idx);}
    };
    ...
    UnloadImage(image);
    if(texture.id==0){TraceLog(LOG_WARNING,"District Fury enemy asset texture creation failed: %s",key);return Texture2D{0};}
